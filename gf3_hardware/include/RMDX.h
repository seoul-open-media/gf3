...
class RMDX{
    public:
        ...
        void reset(uint8_t const& motorId);

        void enable(uint8_t const& motorId);
        void disable(uint8_t const& motorId);

        /// \brief Get the motor acceleration command
        /// \param[in] motorId Motor id
        /// \return Acceleration command, in dps/s^2 ; 0 on failure.
        int32_t getAccelerationCommand(uint8_t const& motorId);

        /// \brief Get the motor position (multiturn)
        /// \param[in] motorId Motor id
        /// \return Position,  command, in dps/s ; -1 on failure.
        int32_t getPosition(uint8_t const& motorId);
        ...
        int setSpeed(uint8_t const& motorId, int32_t const& targetSpeed);
        ...
        int setTorque(uint8_t const& motorId, int32_t const& targetTorque);
