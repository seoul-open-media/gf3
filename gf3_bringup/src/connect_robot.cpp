...
// #include <RMDX.hpp>
namespace gf3_can_driver
{
  class CanConnectorClass
  {
    public:
  ...
  int CanConnectorClass::InitCanInterface(const char *ifname)
  {
    /* 
    * CAN 소켓을 생성한다.
    */
   ...
   * @retval -1: 실패
   */ 
  int CanConnectorClass::TransmitCanFrame(const int sock, const uint32_t id, const uint8_t *data, const size_t data_len)
  {
    /*
    * 전송할 CAN 프레임을 설정한다.
    */
   ...
   * @retval -1: 실패
   */ 
  int CanConnectorClass::ReceiveCanFrame(const int sock)
  {
    /*
    * CAN 프레임을 수신한다.
    */ 
  ...
  int CanConnectorClass::MainLoop()
  {
   ...
   rclcpp::Rate loop_rate(10);
    while(rclcpp::ok())
    {
      uint8_t CAN_ID = 0x142;
      uint8_t can_data[CAN_FRAME_MAX_LEN] = { 0xa2, 0x00, 0x00, 0x00, 0x00, 0xe0, 0x00, 0x00 };
      TransmitCanFrame(sock, 0x142, can_data, sizeof(can_data));
...
int main(int argc, char **argv)
{
  ...
  gf3_can_driver::CanConnectorClass CAN;
